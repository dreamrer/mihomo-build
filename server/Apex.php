...
|
|  【必改 1】加密密钥
|     ★ 第 43 行 ★   private $encryptKey = '';
|     去 Telegram 打包机器人 → 选你的配置 → 「查看加密密钥」复制粘贴。
|     不填或填错 → 客户端解不出节点列表为空。
|
|  【必改 2，仅当客户端用了自定义 UA】协议 flag
|     ★ 第 39 行 ★   public $flag  = 'apex';
|     ★ 第 40 行 ★   public $flags = ['apex'];
...
class Apex extends ClashMeta
{
    ...
    /**
     * V2Board ClientController.php:95-96 把客户端 flag 强制 lowercase 后用
     * str_contains 比对, 机场主写 'Apex' (含大写) 会永远不匹配 → 走 fallback
     * 通用订阅 → 客户端节点空白. 这里统一小写规避。
     *
     * Xboard 的 ProtocolManager 用 stripos 本身不区分大小写, 而且它用
     * newInstanceWithoutConstructor 跳过构造函数 → 这一步对 Xboard 是无操作。
     */
